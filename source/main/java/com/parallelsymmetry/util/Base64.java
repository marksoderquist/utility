...
public final class Base64 {
	...
	public static String encode( String string ) {
		...
		try {
			return encodeBytes( string.getBytes( "UTF-8" ), DONT_BREAK_LINES );
	...
	public static String decode( String string ) {
		...
		try {
			return new String( decodeToBytes( string ), "UTF-8" );
		} catch( UnsupportedEncodingException exception ) {
			Log.write( exception );
			return null;
		}
	 ...
	 * @since 1.3
	 */
	public static byte[] decodeToBytes( byte[] source, int off, int len, int options ) {
	 ...
	 * @since 1.4
	 */
	public static byte[] decodeToBytes( String s, int options ) {
		byte[] bytes;
		try {
		...
		// Decode
		bytes = decodeToBytes( bytes, 0, bytes.length, options );
